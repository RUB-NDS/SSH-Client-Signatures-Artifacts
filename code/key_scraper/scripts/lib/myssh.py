...
def _check_empty(data: bytes) -> None:
    ...
    if data:
        raise ValueError("Corrupt data: unparsed data of length %d" % len(data))
...
def _get_u32(data: memoryview) -> tuple[int, memoryview]:
    """Uint32"""
    if len(data) < 4:
        raise ValueError("Short data parsing u32")
...
def _get_u64(data: memoryview) -> tuple[int, memoryview]:
    """Uint64"""
    if len(data) < 8:
        raise ValueError("Short data parsing u64")
...
def _get_sshstr(data: memoryview) -> tuple[memoryview, memoryview]:
    ...
    if n > len(data):
        raise ValueError("Short data parsing string")
...
def _get_mpint(data: memoryview) -> tuple[int, memoryview]:
    ...
    if val and val[0] > 0x7F:
        raise ValueError("Negative mpint")
...
class _SSHFormatDSA:
    ...
    def load_public(
        ...
        #public_key = public_numbers.public_key()
        public_key = { "alg": "dsa", "p": p, "q": q, "g": g, "y": y }
        return public_key, data
...
class _SSHFormatECDSA:
    ...
    def load_public(
        ...
        #)
        public_key = { "alg": "ecdsa", "curve": self.curve, "point": point.tobytes() }
        return public_key, data
...
class _SSHFormatEd25519:
    ...
    def load_public(
        ...
        #    point.tobytes()
        #)
        public_key = { "alg": "ed25519", "point": point.tobytes() }
        return public_key, data
