...
SRC_INDEX="sshks_users_gitlab"
DEST_INDEX="sshks_users_gitlab_dedup"
MAPPINGS={
  "properties": {
      "deleted": {
        "type": "boolean"
...
def connect_es():
    ...
    if not es.indices.exists(index=DEST_INDEX):
        es.indices.create(index=DEST_INDEX, mappings=MAPPINGS, settings={"number_of_shards": 1, "number_of_replicas": 2})
    return es
